...
int nAnonymizeZevnoAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZEV
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZEV
    // Mac: ~/Library/Application Support/ZEV
    // Unix: ~/.ZEV
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZEV";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Zevno.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "Zevno.conf";
               ...
               fprintf(ConfFile, "port=11222\n");
               fprintf(ConfFile, "rpcport=11221\n");
               fprintf(ConfFile, "rpcconnect=127.0.0.1\n");
               fprintf(ConfFile, "rpcallowip=127.0.0.1\n");
               fprintf(ConfFile, "addnode=5.189.175.51\n");
               fprintf(ConfFile, "addnode=5.189.175.51:11222\n");
               fclose(ConfFile);
    }
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Zevnod.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong Zevno will not work properly.");
