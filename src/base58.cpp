...
namespace {
    class CZevnoAddressVisitor : public boost::static_visitor<bool> {
    private:
        CZevnoAddress *addr;
    public:
        CZevnoAddressVisitor(CZevnoAddress *addrIn) : addr(addrIn) { }
...
bool CZevnoAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CZevnoAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CZevnoAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CZevnoAddressVisitor(this), dest);
}

bool CZevnoAddress::IsValid() const {
...
CTxDestination CZevnoAddress::Get() const {
    if (!IsValid())
...
bool CZevnoAddress::GetKeyID(CKeyID &keyID) const {
...
bool CZevnoAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CZevnoSecret::SetKey(const CKey& vchSecret) {
...
CKey CZevnoSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CZevnoSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CZevnoSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CZevnoSecret::SetString(const std::string& strSecret) {
