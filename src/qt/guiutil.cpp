...
{
    widget->setMaxLength(ZevnoAddressValidator::MaxAddressLength);
    widget->setValidator(new ZevnoAddressValidator(parent));
...
bool parseZevnoURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // NovaCoin: check prefix
    if(uri.scheme() != QString("Zevno"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!ZevnoUnits::parse(ZevnoUnits::ZEV, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseZevnoURI(QString uri, SendCoinsRecipient *out)
{
    // Convert zevno:// to Zevno:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("zevno://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "Zevno:");
    }
    QUrl uriInstance(uri);
    return parseZevnoURI(uriInstance, out);
}
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Zevno.lnk";
}
...
{
    return GetAutostartDir() / "Zevno.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a Zevno.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Zevno\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Zevno app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Zevno app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Zevno-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Zevno-qt [" + tr("command-line options") + "]                     " + "\n";
