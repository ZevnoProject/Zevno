...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", ZevnoUnits::ZEV);
    ...
    if (!settings.contains("nAnonymizeZevnoAmount"))
        settings.setValue("nAnonymizeZevnoAmount", 1000);
    nAnonymizeZevnoAmount = settings.value("nAnonymizeZevnoAmount").toLongLong();
    if (settings.contains("nMNengineRounds"))
        SoftSetArg("-mnenginerounds", settings.value("nMNengineRounds").toString().toStdString());
    if (settings.contains("nAnonymizeZevnoAmount"))
        SoftSetArg("-anonymizeZevnoamount", settings.value("nAnonymizeZevnoAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeZevnoAmount:
            return QVariant(nAnonymizeZevnoAmount);
        case UseDarkTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case MNengineRounds:
            ...
            break;
        case AnonymizeZevnoAmount:
            nAnonymizeZevnoAmount = value.toInt();
            settings.setValue("nAnonymizeZevnoAmount", nAnonymizeZevnoAmount);
            emit AnonymizeZevnoAmountChanged(nAnonymizeZevnoAmount);
            break;
        default:
            break;
        }
