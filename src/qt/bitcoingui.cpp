...
ZevnoGUI::ZevnoGUI(QWidget *parent):
    QMainWindow(parent),
    clientModel(0),
    walletModel(0),
    toolbar(0),
...
{
    resize(900, 520);
    setWindowTitle(tr("Zevno") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    setObjectName("Zevno");
    setStyleSheet("#Zevno {background-image: url(:/images/qt) }");
...
void ZevnoGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Zevno address"));
    ...
    aboutAction = new QAction(QIcon(fUseDarkTheme ? ":/icons/dark/bitcoin-dark" : ":/icons/bitcoin"), tr("&About Zevno"), this);
    aboutAction->setToolTip(tr("Show information about Zevno"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(QIcon(":/qt-project.org/qmessagebox/images/qtlogo-64.png"), tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(QIcon(":/icons/options"), tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Zevno"));
    ...
    editConfigAction = new QAction(QIcon(":/icons/editconf"), tr("&Edit Zevno.conf"), this);
    editConfigAction->setToolTip(tr("Edit the configuration file for Zevno"));
    editConfigExtAction = new QAction(QIcon(":/icons/editconf"), tr("&Edit Zevno.conf (external)"), this);
    editConfigExtAction->setToolTip(tr("Edit the configuration file for Zevno (external editor)"));
    openDataDirAction = new QAction(QIcon(":/icons/folder"), tr("&Open data dir"), this);
    openDataDirAction->setToolTip(tr("Open the directory where Zevno data is stored"));
...
void ZevnoGUI::setClientModel(ClientModel *clientModel)
{
    if(!fOnlyTor)
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Zevno client") + QString(" ") + tr("[testnet]"));
...
void ZevnoGUI::setWalletModel(WalletModel *walletModel)
{
...
void ZevnoGUI::setMessageModel(MessageModel *messageModel)
{
...
void ZevnoGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Zevno client"));
...
#ifndef Q_OS_MAC
void ZevnoGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void ZevnoGUI::setNumConnections(int count)
{
    QString icon;
    switch(count)
    {
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Zevno network", "", count));
}

void ZevnoGUI::setNumBlocks(int count)
{
    QString tooltip;
...
void ZevnoGUI::message(const QString &title, const QString &message, bool modal, unsigned int style)
{
    QString strTitle = tr("Zevno") + " - ";
...
void ZevnoGUI::error(const QString &title, const QString &message, bool modal)
{
...
void ZevnoGUI::changeEvent(QEvent *e)
{
...
void ZevnoGUI::closeEvent(QCloseEvent *event)
{
    if(clientModel)
    {
#ifndef Q_OS_MAC // Ignored on Mac
...
void ZevnoGUI::askFee(qint64 nFeeRequired, bool *payFee)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;
    ...
    QString strMessage = tr("This transaction is over the size limit. You can still send it for a fee of %1, "
        ...
        "Do you want to pay the fee?").arg(ZevnoUnits::formatWithUnit(clientModel->getOptionsModel()->getDisplayUnit(), nFeeRequired));
...
void ZevnoGUI::incomingTransaction(const QModelIndex & parent, int start, int end)
{
    ...
    notificator->notify(Notificator::Information,
                        (amount)<0 ? tr("Sent transaction") :
                          ...
                          tr("Date: %1\n"
                             ...
                             "Address: %4\n")
                          .arg(date)
                          .arg(ZevnoUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(), amount, true))
                          .arg(type)
                          .arg(address), icon);
}

void ZevnoGUI::incomingMessage(const QModelIndex & parent, int start, int end)
{
    if(!messageModel)
        return;
...
void ZevnoGUI::gotoMasternodeManagerPage()
{
...
void ZevnoGUI::gotoBlockBrowser()
{
...
void ZevnoGUI::gotoOverviewPage()
{
...
void ZevnoGUI::gotoAddressBookPage()
{
...
void ZevnoGUI::gotoReceiveCoinsPage()
{
...
void ZevnoGUI::gotoSendCoinsPage()
{
...
void ZevnoGUI::gotoSignMessageTab(QString addr)
{
...
void ZevnoGUI::gotoVerifyMessageTab(QString addr)
{
...
void ZevnoGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void ZevnoGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Zevno address or malformed URI parameters."));
    }

    event->acceptProposedAction();
}

void ZevnoGUI::handleURI(QString strURI)
{
    // URI has to be valid
...
void ZevnoGUI::setEncryptionStatus(int status)
{
...
void ZevnoGUI::importPrivateKey()
{
...
void ZevnoGUI::changePassphrase()
{
...
void ZevnoGUI::showNormalIfMinimized(bool fToggleHidden)
{
...
void ZevnoGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void ZevnoGUI::updateWeight()
{
    if (!pwalletMain)
        return;
...
void ZevnoGUI::updateStakingIcon()
{
    updateWeight();
...
void ZevnoGUI::detectShutdown()
{
    if (ShutdownRequested())
        QMetaObject::invokeMethod(QCoreApplication::instance(), "quit", Qt::QueuedConnection);
}

void ZevnoGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
