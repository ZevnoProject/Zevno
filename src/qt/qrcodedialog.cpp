...
{
    QString ret = QString("Zevno:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ZEV unit input in lnReqAmount, we generate the URI with ZEV as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(ZevnoUnits::format(ZevnoUnits::ZEV, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
