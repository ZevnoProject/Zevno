...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate(): QAbstractItemDelegate(), unit(ZevnoUnits::ZEV)
    {
                      ...
                      const QModelIndex &index ) const
    {
        ...
        painter->setPen(fUseDarkTheme ? QColor(255, 255, 255) : foreground);
        QString amountText = ZevnoUnits::formatWithUnit(unit, amount, true);
        if(!confirmed)
        {
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& stake, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& watchOnlyBalance, const CAmount& watchOnlyStake, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelBalance->setText(ZevnoUnits::formatWithUnit(nDisplayUnit, balance));
    ui->labelStake->setText(ZevnoUnits::formatWithUnit(nDisplayUnit, stake));
    ui->labelUnconfirmed->setText(ZevnoUnits::formatWithUnit(nDisplayUnit, unconfirmedBalance));
    ui->labelImmature->setText(ZevnoUnits::formatWithUnit(nDisplayUnit, immatureBalance));
    ui->labelTotal->setText(ZevnoUnits::formatWithUnit(nDisplayUnit, balance + stake + unconfirmedBalance + immatureBalance));
    ui->labelWatchAvailable->setText(ZevnoUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance));
    ui->labelWatchStake->setText(ZevnoUnits::floorWithUnit(nDisplayUnit, watchOnlyStake));
    ui->labelWatchPending->setText(ZevnoUnits::floorWithUnit(nDisplayUnit, watchUnconfBalance));
    ui->labelWatchImmature->setText(ZevnoUnits::floorWithUnit(nDisplayUnit, watchImmatureBalance));
    ui->labelWatchTotal->setText(ZevnoUnits::floorWithUnit(nDisplayUnit, watchOnlyBalance + watchOnlyStake + watchUnconfBalance + watchImmatureBalance));
...
void OverviewPage::setWalletModel(WalletModel *model)
{
    ...
    // update the display unit, to not use the default ("ZEV")
    updateDisplayUnit();
}
